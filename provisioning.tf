...
resource "null_resource" "install_exporters" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "[Service]",
      "ExecStart=/usr/local/bin/mongodb_exporter --mongodb.uri=mongodb://localhost:27018/?directConnection=true --mongodb.direct-connect=true --mongodb.collstats-colls=sensordb.readings --collect-all --web.listen-address=:9216",
      "Restart=always",
      "[Install]",
...
resource "null_resource" "install_monitoring" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "    honor_labels: false",
      "    static_configs:",
      "      - targets: ['${aws_instance.mongo_eu.public_ip}:9216']",
      "        labels:",
      "          region: 'eu-west-1'",
      "          shard: 'shard2'",
      ...
      "    honor_labels: false",
      "    static_configs:",
      "      - targets: ['${aws_instance.mongo_ap.public_ip}:9216']",
      "        labels:",
      "          region: 'ap-south-1'",
      "          shard: 'shard3'",
      "EOF",
      ...
      "[Service]",
      "User=prometheus",
      "ExecStart=/usr/local/bin/prometheus --config.file=/etc/prometheus/prometheus.yml --storage.tsdb.path=/var/lib/prometheus/ --web.enable-lifecycle --storage.tsdb.retention.time=3d",
      "Restart=always",
      "[Install]",
      "WantedBy=multi-user.target",
      "EOF",
